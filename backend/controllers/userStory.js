...
const saveUserStory = async (req, res) => {
  if (!req.body.name || !req.body.userId || !req.body.userStoryStatus || !req.body.details)
...
const listUserStoryAdmin = async (req, res) => {
  ...
  if(isAdmin.name === 'admin'){
    if(!req.body.userId) return res.status(400).send({message: "Incomplete data"});
...
const updateUserStoryAdmin = async (req, res) => {
  if (!req.body.bugs || !req.body.userStoryStatus || !req.body.name || !req.body.userId || !req.body.registerDate || !req.body.details)
    return res.status(400).send({ message: "Incomplete data" });

    const userStoryExists = await userStory.findById({_id: req.body._id})
    if(!userStoryExists) return res.status(400).send({ message: "User story not exists" });
...
const updateUserStory = async (req, res) => {
  if (!req.body.bugs || !req.body.userStoryStatus)
    ...
    const userStoryExists = await userStory.findById({_id: req.body._id})
    const {userId, name, description, registerDate, details, userStoryStatus, bugs} = userStoryExists;
...
const deleteUserStory = async (req, res) => {
  const userStoryDelete = await userStory.findByIdAndDelete({ _id: req.params["_id"] });
  if (!userStoryDelete) return res.status(400).send({ message: "User story not found" });
...
export default { saveUserStory, listUserStory, updateUserStory, deleteUserStory, listUserStoryAdmin, updateUserStoryAdmin };
